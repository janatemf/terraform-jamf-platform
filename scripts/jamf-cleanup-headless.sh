...
delete_jamf_objects(){
    ...
    # Delete API integrations/clients (API v1)
    echo "Deleting API integrations..."
    response=$(curl -s -H "Authorization: Bearer ${bearerToken}" -X GET "$jamfURL/api/v1/api-integrations")
    idList=$(echo "$response" | python3 -c "import sys, json; data=json.load(sys.stdin); print(' '.join(str(item['id']) for item in data.get('results', [])))" 2>/dev/null)

    if [ -n "$idList" ]; then
        echo "Found API integration IDs: $idList"
        for id in $idList; do
            http_code=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer ${bearerToken}" -X DELETE "$jamfURL/api/v1/api-integrations/$id")
            if [[ "$http_code" == "204" ]] || [[ "$http_code" == "200" ]]; then
                echo "Successfully deleted API integration: $id"
                let successful+=1
            else
                echo "Failed to delete API integration: $id (HTTP $http_code)"
                let failures+=1
            fi
            let total+=1
        done
    else
        echo "No API integrations found to delete"
    fi

    # Delete API roles (API v1)
    echo "Deleting API roles..."
    response=$(curl -s -H "Authorization: Bearer ${bearerToken}" -X GET "$jamfURL/api/v1/api-roles")
    idList=$(echo "$response" | python3 -c "import sys, json; data=json.load(sys.stdin); print(' '.join(str(item['id']) for item in data.get('results', [])))" 2>/dev/null)

    if [ -n "$idList" ]; then
        echo "Found API role IDs: $idList"
        for id in $idList; do
            http_code=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer ${bearerToken}" -X DELETE "$jamfURL/api/v1/api-roles/$id")
            if [[ "$http_code" == "204" ]] || [[ "$http_code" == "200" ]]; then
                echo "Successfully deleted API role: $id"
                let successful+=1
            else
                echo "Failed to delete API role: $id (HTTP $http_code)"
                let failures+=1
            fi
            let total+=1
        done
    else
        echo "No API roles found to delete"
    fi
